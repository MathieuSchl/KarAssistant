...
module.exports.execute = ({ query, lang, data }) => {
  if (/^\d+?$/.test(query)) {
    ...
    const textResult = userValue === gameTarget ? text.response[lang].correctValue : userValue < gameTarget ? text.response[lang].greaterValue : text.response[lang].smallerValue;
